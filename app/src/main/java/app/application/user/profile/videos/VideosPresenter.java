...
/**
 * Listens to user actions from the UI ({@link VideosFragment}), retrieves the data and updates
 * the UI as required.
 */
public class VideosPresenter extends BasePresenter<VideosContract.View> implements VideosContract.Presenter {
    ...
    public void getVideos(Context context) {
        checkViewAttached();
        getView().showLoading();
        addSubscription(videosRepository.getVideos(context) .subscribeOn(Schedulers.io())
                ...
                .subscribe(new Subscriber<List<VideosModel>>() {
                    ...
                    public void onNext(List<VideosModel> videosModel) {
                        ...
                        if(userVideos.size() == 0){
                            getView().showError("There are no videos");
                        } else {
                            getView().showVideos(userVideos);
                        }
                    }
